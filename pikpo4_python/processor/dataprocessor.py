...
class DataProcessor(ABC):
    ...
    def run(self) -> None:
        ...
        for (columnName, columnData) in self.result.iteritems():
            if (columnName == 'country'):
                continue
            mas = self.result[columnName]
            count = -1
            for item in mas:
                count = count + 1
                x = str(item)
                if x[-1] == "k":
                    mas[count] = x.replace('k', '000').replace('.', '')
                elif x[-1] == "M":
                    mas[count] = x.replace('M', '000000').replace('.', '')
                elif x[-1] != '0':
                    mas[count] = -1
                mas[count] = int(mas[count])
            self.result.replace(self.result[columnName], mas)
        print(self.result)
    ...
    def read(self):
        try:
            ...
            for separator in self.separators:
                self._dataset = pandas.read_csv(self._datasource, on_bad_lines='skip', sep=separator, header='infer',
                                                names=None,
                                                encoding="utf-8")
...
class TxtDataProcessor(DataProcessor):
    ...
    def read(self):
        ...
        try:
            self._dataset = pandas.read_table(self._datasource, sep=' ', engine='python', on_bad_lines='skip')
            col_names = self._dataset.columns
            if len(col_names) < 2:
                return False
            return True
